...
case class State(t: Term, ρ: Env, σ: Store, κ: Kont, tr: Trace) {
  ...
  def next: Set[State] = t match {
    case StmtT(stmt) ⇒ stmt match {
      case e: Exp ⇒
        val v = eval(e)
        if (v.notEmpty) State(eval(e), ρ, σ, κ, tr)
        else Set()

      case Seq(s :: rest) ⇒
        State(s, ρ, σ, seqK(rest, κ), tr)
      ...
      case If(e: Exp, s1: Stmt, s2: Stmt) ⇒
        ...
        if (guard.definitelyTrue)
          ...
          Set(State(s1, ρ, σ, κ, tr), State(s2, ρ, σ, κ, tr))
        else
          Set()
      ...
      case Call(x, ef, es) ⇒
        ...
        for (Closure(ρc, Fun(xs, s)) <- closureSet) {
          val as = xs map ((x: Var) ⇒ Address(tr + x.lbl))
          val vs = es map eval
          val l = Address(tr + s.lbl)
    ...
    case ValueT(v) ⇒ κ match {
      ...
      case whileK(e, s, κc) ⇒
        val guard = eval(e)
        if (guard.definitelyTrue)
          State(s, ρ, σ, whileK(e, s, κc), tr)
        else if (guard.definitelyFalse)
          State(guard, ρ, σ, κc, tr)
        else if (guard.notEmpty)
          Set(State(s, ρ, σ, whileK(e, s, κc), tr), State(guard, ρ, σ, κc, tr))
        else
          Set()
